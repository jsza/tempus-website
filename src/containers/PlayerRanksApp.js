...
class PlayerRanksApp extends React.Component {
  ...
  loadRanks() {
    ...
    this.props.loadRanks(this.props.route.rankType, start)
  }

  componentDidUpdate(prevProps) {
    const oldRankType = prevProps.route.rankType
    const newRankType = this.props.route.rankType
    const {start} = this.props.location.query
    const oldStart = prevProps.location.query.start

    if (oldRankType !== newRankType || oldStart !== start) {
      this.loadRanks()
    }
  }

  renderTitle() {
    const {rankType} = this.props.route
    let icon
    if (rankType === 'overall') {
      icon = (
